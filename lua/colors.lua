...
-----------------------------------------------------------------------------
-- Was edited by Aurelio Buonomo (https://github.com/auribuo) to have type hints and work in his neovim config
...
---@param H number|string  hue (0-360) _or_ an RGB string ("#930219")
...
---@param s number                  saturation (0.0-1.0)
---@param L number                  lightness (0.0-1.0)
---@return number,number,number     # an R, G, and B component of RGB
...
function Color:to_rgb()
    ...
    for _, v in ipairs(rgb) do
        -- old version math.floor(v * 256 + 0.5) could produce hex codes over 2 digits
        buffer = buffer .. string.format("%02x", clamp(math.floor(v * 255 + 0.5), 0, 255))
    end
    return buffer
end
...
---@param angle number  the difference in hue between this color and the neighbors
...
---@param angle number  the difference in hue between the complementary and the returned colors
---@return Color,Color  # two new instances of Color
...
---@param f fun(color: Color, seq: integer, max: integer) the function to create variations
---@param n integer                                       the number of variations
...
---@param n integer the number of tints
---@return table    # a table with n values containing the new colors
...
---@param n integer the number of shades
---@return table    # a table with n values containing the new colors
